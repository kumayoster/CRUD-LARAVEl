...
<div class="max-w-[720px] mx-auto py-16">
    ...
    <form action="{{ route('barangkeluar.store') }}" method="POST">
        ...
        <div class="mb-4">
            ...
            <input type="date" name="tgl_keluar" id="tgl_keluar" class="border px-4 py-2 w-full" required>
        </div>

        <div class="mb-4">
            <label for="jml_keluar" class="block text-sm">Jumlah Keluar</label>
            <input type="number" name="jml_keluar" id="jml_keluar" class="border px-4 py-2 w-full" required min="1">
        </div>

        <div class="mb-4">
            <label for="lokasi" class="block text-sm">Lokasi</label>
            <input type="text" name="lokasi" id="lokasi" class="border px-4 py-2 w-full" required>
        </div>

        <div class="mb-4">
            <label for="penerima" class="block text-sm">Penerima</label>
            <input type="text" name="penerima" id="penerima" class="border px-4 py-2 w-full" required>
        </div>

        <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded">Tambah Barang Keluar</button>
    </form>
</div>
@endsection
